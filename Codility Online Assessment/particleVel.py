...
consecutive position measurements remains the same. Note that you need at least three measurements 
to be sure that the particle didn't change its velocity.
...
- More formally, your task is to find all the periods of time particles[P], particles[P+1], ....particles[Q] (of length at least 3) 
during which the movement of the particle is stable. Note that some periods of time might be contained in others (see below example).
...
def particleVelocity(particles):
    ...
    for i in range(len(particles) - 2):
        for j in range(I + 1, len(particles) - 1):
            if particles[j + i] - particles[j] == particles[i + 1] - particles[i]:
                numStable += 1
            else:
                break
